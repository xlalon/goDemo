...
func (w *Waxp) scanTxnByAccount(ctx context.Context, cursor *xchain.Cursor) ([]*xchain.Transfer, error) {
	var txs []*xchain.Transfer
	resp, err := w.getActions(ctx, cursor.Address, 1, 100)
	if err != nil {
		return txs, err
	}
	actionsArray := json.JGet(resp, "actions").Array()
	for _, action := range actionsArray {
		if txn, errParsed := w.parseAct(action.String()); txn != nil && errParsed == nil && txn.Recipient.Address == cursor.Address {
			txs = append(txs, txn)
		}
	}
...
func (w *Waxp) NewAccount(ctx context.Context, walletLabel xchain.WalletLabel) (*xchain.AccountDTO, error) {
	_ = ctx
	account := &xchain.AccountDTO{}
...
func (w *Waxp) GetWalletBalance(ctx context.Context, walletLabel xchain.WalletLabel, identity xchain.Identity) (xchain.Coin, error) {
	_ = ctx
	_ = walletLabel
	_ = identity
	return xchain.Coin{Identity: identity, Amount: decimal.Zero()}, nil

}

func (w *Waxp) GetAccountBalance(ctx context.Context, address xchain.Address, identity xchain.Identity) (xchain.Coin, error) {
	balance := xchain.Coin{Identity: identity, Amount: decimal.Zero()}
